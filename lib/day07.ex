defmodule Aoc23.Day07 do
  ...
  def hands_reducer(hands) do
    for {%{bid: bid}, i} <- Enum.zip([hands, 1..(length(hands))]), reduce: 0 do
      acc -> acc + bid * i
    end
  end
...
defmodule CamelCardsHand do
  defstruct cards: [], bid: 0
  ...
  defp compare_helper({c1, c2}, _) do
    cond do
      c1 > c2 -> {:halt, true}
      c1 < c2 -> {:halt, false}
      true -> {:cont, true}
  ...
  def compare(%{cards: h1}, %{cards: h2}) do
    cond do
